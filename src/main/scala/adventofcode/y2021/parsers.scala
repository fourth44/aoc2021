...
object Parser {
  ...
  val num: Parser[Int] = regex("-?\\d+".r).map(_.toInt)
  ...
  def repsep[A](sep: Parser[String], p: Parser[A]): Parser[Seq[A]] =
    opt(p ** opt(sep *> repsep[A](sep, p)))
      .map {
        _.toSeq.flatMap((head, tail) => head +: tail.toSeq.flatten)
      }

  def csv[A](p: Parser[A]): Parser[Seq[A]] = repsep(string(","), p)

  def bracketed[A](p: Parser[A]): Parser[A] =
    (string("[") ** p ** string("]")).map { case ((_, a), _) => a }
    ...
    def flatMap[B](f: A => Parser[B]): Parser[B] = loc => {
      p(loc) match
        case Result.Success(a, charsConsumed) =>
          // We have to advance the success by the initial characters consumed,
          // otherwise the characters consumed by p are forgotten.
          f(a)(Location(loc.input, loc.offset + charsConsumed))
